...
it('should call save with correct values when addUpdateLog is called', async () => {
  // Arrange
  ...
  // Act
  await auditLogService.addUpdateLog(AUDIT_LOG_DTO);
  ...
  expect(auditLogRepositoryMock.create).toHaveBeenCalledWith({
    ...
    actionType: EAuditActionType.UPDATE,
  });
