...
it('should call save with correct values when addActionLog is called', async () => {
  // Arrange
  ...
  // Act
  await auditLogService.addActionLog(AUDIT_LOG_DTO);
  ...
  expect(auditLogRepositoryMock.create).toHaveBeenCalledWith({
    ...
    actionType: EAuditActionType.ACTION,
  });
